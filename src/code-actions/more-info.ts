...
export class MoreInfo implements vscode.CodeActionProvider {
	...
	provideCodeActions(document: vscode.TextDocument, range: vscode.Range | vscode.Selection, context: vscode.CodeActionContext, token: vscode.CancellationToken): vscode.CodeAction[] {
		return context.diagnostics
            ...
            .filter(diagnostic => getViolationFromDiagnostic(diagnostic) && getViolationFromDiagnostic(diagnostic)?.ruleUrl)
	...
	private createCommandCodeAction(diagnostic: vscode.Diagnostic): vscode.CodeAction {
		...
		action.command = { command: LEARN_MORE_COMMAND, arguments: [violation?.ruleUrl], title: 'Learn more', tooltip: 'This will open your browser with documentation on this violation.' };
